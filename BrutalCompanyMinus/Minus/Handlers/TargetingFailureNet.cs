...
{
    [HarmonyPatch]
    internal class TargetingFailureNet : NetworkBehaviour
    {
        public static TargetingFailureNet instance;

        public void Awake()
        {
            if (instance != null) DestroyInstance();
            instance = this;
            Net.Instance.SetEventActiveServerRPC("TargetingFailureEvent", true);
        }

        public static void DestroyInstance() // This handles the deletion of Time Chaosness
        {
            Events.TargetingFailureEvent.Instance.Active = false;
            GameObject netObject = GameObject.Find("TargetingFailureEvent");
            if (netObject != null)
            {
