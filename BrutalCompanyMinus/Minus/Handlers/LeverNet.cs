...
namespace BrutalCompanyMinus.Minus.MonoBehaviours
{
    ...
    {
        public static LeverNet instance;

        public void Awake()
        {
            if (instance != null) DestroyInstance();
            instance = this;
            Net.Instance.SetEventActiveServerRPC("LeverFailure", true);
        }

        public static void DestroyInstance() // This handles the deletion of Time Chaosness
        {
            Events.LeverFailure.Instance.Active = false;
            GameObject netObject = GameObject.Find("LeverEvent");
            if (netObject != null)
            {
