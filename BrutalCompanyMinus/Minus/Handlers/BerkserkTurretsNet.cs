...
{
    [HarmonyPatch]
    internal class BerserkTurretsNet : NetworkBehaviour
    {
        public static BerserkTurretsNet instance;

        public void Awake()
        {
            if (instance != null) DestroyInstance();
            instance = this;
            Net.Instance.SetEventActiveServerRPC(nameof(BerserkTurrets), true);
        }

        public static void DestroyInstance() // This handles the deletion of Time Chaosness
        {
            Events.BerserkTurrets.Instance.Active = false;
            GameObject netObject = GameObject.Find("BerserkTurretsEvent");
            if (netObject != null)
            {
