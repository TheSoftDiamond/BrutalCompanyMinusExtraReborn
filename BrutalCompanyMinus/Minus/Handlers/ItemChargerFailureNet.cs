...
{
    [HarmonyPatch]
    internal class ItemChargerFailureNet : NetworkBehaviour
    {
        public static ItemChargerFailureNet instance;

        public void Awake()
        {
            if (instance != null) DestroyInstance();
            instance = this;
            Net.Instance.SetEventActiveServerRPC(nameof(ItemChargerFailure), true);
        }

        public static void DestroyInstance() // This handles the deletion of Time Chaosness
        {
            Events.ItemChargerFailure.Instance.Active = false;
            GameObject netObject = GameObject.Find("ItemChargerFailureEvent");
            if (netObject != null)
            {
